...
#test both hand have different combination, no tie 
  test "straightFlush against four" do
    assert PokerBb.winner(straightFlush(),four()) == 1
  end
  test "four of a kind against fullHouse" do
    assert PokerBb.winner(four(),fullHouse()) == 1
  end
  test "fullHouse against flush" do
    assert PokerBb.winner(fullHouse(),flush()) == 1
  end
  test "flush against straight" do
    assert PokerBb.winner(flush(),straight()) == 1
  end
  test "straight against three of a kind" do
    assert PokerBb.winner(straight(),three()) == 1
  end
  test "three against two pairs" do
    assert PokerBb.winner(three(),twoPair()) == 1
  end
  test "two pairs against pair" do
    assert PokerBb.winner(twoPair(),pair()) == 1
  end
  test "pair against high card" do
    assert PokerBb.winner(pair(),high()) == 1
  end

#test tie breaker
test "straightFlush tiebreaker" do
  assert PokerBb.winner(straightFlush(),straightFlush2()) == 1
end
test "four tiebreaker" do
  assert PokerBb.winner(four(),four2()) == 1
end
test "fullHouse tiebreaker" do
  assert PokerBb.winner(fullHouse(),fullHouse2()) == 1
end
test "flush tiebreaker" do
  assert PokerBb.winner(flush(),flush2()) == 1
end
test "straight tiebreaker" do
  assert PokerBb.winner(straight(),straight2()) == 1
end
test "three of a kind tiebreaker" do
  assert PokerBb.winner(three(),three2()) == 1
end
test "two pairs tiebreaker if both hands have two pairs with same cards for first pair" do
  assert PokerBb.winner(twoPair(),twoPair2()) == 1
end
test "two pairs tiebreaker if both hands have two pairs with same cards for both pairs" do
  assert PokerBb.winner(twoPair(),twoPair3()) == 1
end
test "two pairs tiebreaker if both hands have two pairs, first hand has higher first pair" do
  assert PokerBb.winner(twoPair(),twoPair4()) == 1
end
test "pair tiebreaker if both hands have a pair, first hand  has highed pair" do
  assert PokerBb.winner(pair(),pair2()) == 1
end
test "pair tiebreaker if both hands have a pair with same value" do
  assert PokerBb.winner(pair(),pair3()) == 1
end
test "high card tiebreaker, first card is the same" do
  assert PokerBb.winner(high(),high2()) == 1
end
test "high card tiebreaker, all cards are the same except last one" do
  assert PokerBb.winner(high(),high3()) == 1
end
